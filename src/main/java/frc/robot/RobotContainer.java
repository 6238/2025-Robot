...
public class RobotContainer {
     ...
     * edu.wpi.first.wpilibj2.command.button.Trigger}s, such as controller
     * buttons and subsystem state, to
     * {@link edu.wpi.first.wpilibj2.command.Command} instances. The advantage
     * of configuring all the robot's logic here is that it's easy to find, and
     * therefore easy to modify, what the robot does when something happens and
     * why.
     */
    private void configureTriggers() {
        ...
        driverXbox
                ...
                .whileTrue(
                        Commands.sequence(
                                Commands.defer(
                                        () -> swerve.align(new APTarget(ReefUtils.GetBargePose(swerve.getPose())).withEntryAngle(Rotation2d.kZero)),
                                        Set.of(swerve)),
                                m_elevator.setHeightCommand(ElevatorHeights.TOP),
                                Commands.waitUntil(() -> m_elevator.getHeight() > ElevatorHeights.TOP - 4)));
